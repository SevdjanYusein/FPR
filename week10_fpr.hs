...
maximize fs = foldl1 fmaxabs fs

fmaxabs f g = \x -> if abs (f x) > abs (g x) 
                    then f x 
                    else g x
fmax f g = \x -> max (f x) (g x)

-- = foldl1 (\f1 f2 -> (\x -> if f1 x > f2 x then f1 x else f2 x)) fs
-- = maximize' fs = foldl1 (\f1 f2 -> (\x -> max (f1 x) (f2 x)) fs
-- = (\x -> maximum (map (\f -> f x) fs))
