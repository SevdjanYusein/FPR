...
boundUp f up = (\x -> if (f x > up) then up else f x )       -- (\x -> min (f x) up)

boundDown :: (Double -> Double) -> Double -> (Double -> Double)
boundDown f down = (\x -> if (f x < down) then down else f x) -- f max f (const down)
...
maximize (f:[]) = f
maximize (f:fs) = (\x -> if abs (f x) > abs (maximize fs x) 
                         then f x 
                         else maximize fs x)
...
closestPoint ps = (\inputPoint -> argmin (`dist` inputPoint) ps )
                    where dist (x1, y1) (x2, y2) = sqrt $ (x1 - x2) ^ 2 + (y1 - y2) ^ 2 
                    --dist = (\(px, py) -> sqrt ((px - xi) ** 2 + (py - yi) ** 2))
