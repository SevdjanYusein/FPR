{- Упражнение 5. Tuples, List comprehension, zip/zipWith -}
import Data.Char
...
anyDigitAtEvenPos str = or [isDigit c | (i, c) <- enumerate str, even i]
...
indices x xs = [i | (i, x') <- enumerate xs, x' == x]
...
countMin xs = lenght [x | x <- xs, x == minimum xs] -- lenght(filter(\x-> x==minimum)xs)
