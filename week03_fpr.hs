...
-- Задача 5. функцията isPerfectNumber n, която проверява дали цялото число n е перфектно.
...
-}
countOfDigits :: Integer -> Integer
countOfDigits n
              | (n >= 0) && (n <= 10) = 1
              | n >= 10 =  1 + (countOfDigits (getFirstDigits n))
              
isNarcissistic :: Integer -> Bool
isNarcissistic n = if (n ## (countOfDigits n)) == n
                   then True
                   else False
