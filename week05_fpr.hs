...
encrypt "" _ = ""
encrypt (x:xs) n = newLetter : encrypt xs n
    where newLetter = chr ((ord x - ord 'A' + n) `mod` 26 + ord 'A')

decrypt :: String -> Int -> String
decrypt"" _ = ""
decrypt (x:xs) n = newLetter' : decrypt xs n
    where newLetter' = chr ((ord x - ord 'A' - n) `mod` 26 + ord 'A')
...
joinWords _ [] = ""
joinWords _ (xs:[]) = xs
joinWords c (xs:xss) = xs ++ [c] ++ joinWords c xss 
...
rotate :: Int -> [a] -> [a]
rotate _ [] = []
rotate 0 str = str     -- дъно
rotate n (x:xs) = rotate (n-1) (xs ++ [x])

rotations :: [a] -> [[a]]
rotations [] = []
rotations xs = helper 0 
    where helper i = if i == length xs
                then []
                else rotate i xs : helper (i + 1)

bwt :: Ord a => [a] -> [a]
bwt xs = helper (sort $ rotations xs)
    where helper [] = []
          helper (xs:xss) = lastElementList : helper xss
            where lastElementList = last xs
